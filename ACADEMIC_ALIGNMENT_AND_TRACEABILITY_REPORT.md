...
* **Slide 4 (3:00–4:00)**: *"However, the authors noted in the Appendix (Limitations) that multimodal models remained an open problem. In VLMs, safety is significantly more fragile because visual tokens bypass text safety filters. An instruction that would be refused in text can easily bypass the model when rendered as typography in an image, as shown in FigStep."*
