...
SCRIPT_DATA = {
    1: {
        ...
        "qa": "Q: Why extend this to VLMs specifically?\nA: In the Limitations section of Bach et al., you explicitly noted that extending gradient selection to VLMs remains an open question. In VLMs, the visual channel introduces an entirely new, unaligned attack surface where typography and visual jailbreaks bypass text filters completely.",
    ...
    3: {
        ...
        "qa": "Q: How does elasticity apply to multimodal models?\nA: In VLMs, the restoring force exists in both the language model and the vision-language projector. Cross-modal tension stretches the projector weights, causing rapid forgetting of aligned refusal boundaries.",
    ...
    4: {
        ...
        "qa": "Q: Why use 1D VISAGE instead of 2D?\nA: 1D VISAGE captures the exact same model safety ranking as 2D (Llama 3 > Llama 2 > Mistral > Vicuna) at a fraction of the compute cost, making it ideal for tracking drift across continual training stages.",
    ...
    6: {
        ...
        "qa": "Q: What is cross-modal conflict?\nA: An attack where the image and text prompts are individually benign or ambiguous, but their cross-modal combination forms a harmful instruction that bypasses unimodal safety filters.",
        "transition": "Next, let's examine how prior works have attempted to defend VLMs, specifically SafeVLM."
    },
    7: {
        "title": "Slide 7: Related Multimodal Safety Approach: SafeVLM",
    ...
    8: {
        "title": "Slide 8: SafeVLM Empirical Results & Over-Refusal Trade-offs",
...
def inject_notes():
    ...
    for slide_num, data in SCRIPT_DATA.items():
        ...
        full_note = (
            f"=== {data['title']} ===\n\n"
            f"[SPOKEN SCRIPT - READ THIS OUT]:\n"
            f"\"{data['script']}\"\n\n"
            f"[ANTICIPATED QUESTION & DEFENSE]:\n"
            f"{data['qa']}\n\n"
    ...
    print("\nSuccessfully updated Presentation1.pptx with all 31 slide notes!")
